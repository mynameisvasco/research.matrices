  ...
  ({
    ...
    return (
      <rect
        ...
        y={strokeWidth / 2}
        fill={
          shapeIndex === highlighted_box && shapeIndex !== undefined
            ? "rgba(233, 244, 10, 0.1)"
            : "rgba(0,255,255,0.1)"
        }
        stroke={
...
class Editor extends Component {
  ...
  start_segmentation = () => {
    ...
    let url =
      "http://deti-iforal.ua.pt:5015/mybiros/api/v1/text-detection/image/";
    axios
      ...
      .then((response) => {
        ...
        this.props.async_set_segmentation_boxes(
          index_file,
          boxes_from_segmentation
        );
        this.setState({
          items: boxes_from_segmentation,
          id: boxes_from_segmentation.length + 1,
          loading: false,
        });
  ...
  set_highlighted_rect_id = (ID) => {
    this.state.highlighted_box !== ID
      ? this.setState({ highlighted_box: ID }, () => {
          this.props.set_highlight_box_on_rect_click(ID);
        })
      : this.setState({ highlighted_box: -1 }, () => {
          this.props.set_highlight_box_on_rect_click(-1);
        });
  };
  ...
  duplicate_on_keyboard_press = (event) => {
    if (event.ctrlKey && event.key === "c") {
      let found = this.state.items.find(
        (element) => element.id === this.state.highlighted_box
      );
      if (found !== undefined) {
        var new_object = {
          id: this.state.id.toString(),
          x: found.x,
          y: found.y + found.height + 5,
          width: found.width,
          height: found.height,
        };
        this.setState(
  ...
  render() {
      ...
      return (
        <IssueWrapper>
          <div className="text_placeholder">
            Please select an image in the gallery
          </div>
        </IssueWrapper>
      );
    }
    ...
    const shapes = this.state.items.map((item, index) => {
      ...
      return (
        <RectShape
          ...
          onDelete={() => {
            let newArrayofItems = this.arrayReplace(
              this.state.items,
              index,
              []
            );
          ...
          ResizeHandleComponent={(_a) => {
            var active = _a.active,
              cursor = _a.cursor,
              isInSelectionGroup = _a.isInSelectionGroup,
              onMouseDown = _a.onMouseDown,
              recommendedSize = _a.recommendedSize,
              scale = _a.scale,
              x = _a.x,
              y = _a.y;
            return React.createElement("rect", {
              fill: active ? "rgba(229,240,244,1)" : "rgba(229,240,244,0)",
              height: recommendedSize,
              stroke: active ? "rgba(53,33,140,1)" : "rgba(53,33,140,0)",
              strokeWidth: 1 / scale,
              style: { cursor: cursor, opacity: isInSelectionGroup ? 0 : 1 },
    ...
    return (
      <div onKeyUp={this.duplicate_on_keyboard_press}>
        <Outer>
          <Toolbar>
            <Button basic compact onClick={() => changeScale(Math.sqrt(1.25))}>
              <FaSearchPlus />
            </Button>
            <Button
              basic
              compact
              onClick={() => changeScale(1 / Math.sqrt(1.25))}
            >
              <FaSearchMinus />
            </Button>

            <Button basic compact onClick={this.start_segmentation}>
              Segmentation
            </Button>
            ...
            <Button
              ...
              id="botn"
              toggle
              active={this.state.is_panning === false}
              compact
              onClick={() => this.setState({ is_panning: false })}
            >
            ...
            <Button
              ...
              id="botn"
              toggle
              active={this.state.is_panning === true}
              compact
              onClick={() => this.setState({ is_panning: true })}
            >
              <FaHandPointer />
            </Button>
          </Toolbar>
          <Container>
            <PanZoom
              ...
              disableScrollZoom
              disabled={this.state.is_panning === false}
            >
              <InnerContainer style={{ width: vectorWidth * scale }}>
                ...
                >
                  <ShapeEditor
                    ...
                    style={{
                      pointerEvents: this.state.is_panning ? "none" : "auto",
                      display: "block",
                  ...
                  >
                    ...
                    <DrawLayer
                      ...
                      onAddShape={({ x, y, width, height }) => {
                        this.setState((state) => {
                          const items_befor_sort = state.items.concat({
                          ...
                          var items = items_befor_sort.sort(
                            (a, b) => a.y - b.y
                          );
                          return {
                            items,
