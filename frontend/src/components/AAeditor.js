...
class MultiPageEditor extends React.Component {
  ...
  togglefilter0 = () => {
    ...
    let url =
      "http://deti-iforal.ua.pt:5015/mybiros/api/v1/text-detection/corpus/";
    axios
      .post(url, form_data, {
        headers: {
      ...
      axios
        ...
        .then((response) => {
          ...
          for (let j = 0; j < this.state.files[i].boxes.length; j++) {
            let elem = {
              id: this.state.files[i].boxes[j].id,
              text: response.data.predictions[j]
                ? response.data.predictions[j]
                : "",
            };
            lista.push(elem);
          }
  ...
  premilo = () => {
    let lisii = [];
    this.state.files[this.state.index_active_file_in_files].boxes.forEach(
      (box) => lisii.push({ id: box.id, text: "Ferdinando II" })
    );
    this.setState({
      files: update(this.state.files, {
        [this.state.index_active_file_in_files]: {
          text_response: { $set: lisii },
        },
      }),
    });
  };
          ...
          <Frame
            ...
            onClick={(v) =>
              this.setState({
                ...
                list_length: value.boxes.length,
              })
            }
          >
            ...
            <div>
                ...
                <div className="container">
                  <div>
                    <Button.Group basic widths="2">
                      <Button id="botn" compact onClick={this.togglefilter0}>
                        Segment ALL images
                      </Button>
                      <Button id="botn" compact onClick={this.togglefilter1}>
                        Transcribe ALL images
                      </Button>
                      <Button
                        id="botn"
                        compact
                        onClick={this.callDatasetCreator}
                      >
                        Download Lines Result
                      </Button>
                    </Button.Group>
                  </div>
              ...
              >
                <Editor
                  ...
                  async_set_segmentation_boxes={
                    this.async_set_segmentation_boxes
                  }
                  ...
                  <>
                    <Toolbar>
                      <Button
                        ...
                        id="botn"
                        compact
                        onClick={this.download_collection}
                      >
                        Download all .txt
                      </Button>
                    </Toolbar>
