...
class SinglePage extends React.Component {
  ...
  jsPdfGenerator = () => {
    ...
    if (this.state.file !== null && this.state.bounding_boxes.length) {
      ...
      if (
        this.state.image_dimensions.width >= this.state.image_dimensions.height
      ) {
        template = "l";
        scaled_image_values = {
          width: 632,
          height:
            (this.state.image_dimensions.height * 632) /
        ...
        scaled_image_values = {
          width: 447,
          height:
            (this.state.image_dimensions.height * 447) /
            this.state.image_dimensions.width,
        };
      }
      var doc = new jsPDF(template, "pt", [
        scaled_image_values.width,
        scaled_image_values.height,
      ]);
      ...
      for (let i = 0; i < this.state.bounding_boxes.length; i++) {
        ...
        let new_width =
          (this.state.bounding_boxes[i].width * scaled_image_values.width) /
          this.state.image_dimensions.width;
        let font_size = new_width / string_unit;

        let new_x =
          (this.state.bounding_boxes[i].x * scaled_image_values.width) /
          this.state.image_dimensions.width;
        let new_y =
          (this.state.bounding_boxes[i].y * scaled_image_values.height) /
            this.state.image_dimensions.height +
          (this.state.bounding_boxes[i].height * scaled_image_values.height) /
            this.state.image_dimensions.height;
  ...
  sendPage2HTR_new = (e) => {
    ...
    let url_predict = "http://deti-iforal.ua.pt:5050/predict";
    axios
      ...
      .then((response) => {
        ...
        for (let i = 0; i < this.state.bounding_boxes.length; i++) {
          let elem = {
            id: this.state.bounding_boxes[i].id,
            text: response.data.predictions[i].prediction,
          };
          lista.push(elem);
        }
        this.raw_text_update(lista);
        this.setState({ text_resp_length: lista.length }, () =>
          this.setState(
            { text_response: lista, loading: false },
            this.jsPdfGenerator
          )
        );
      })
  ...
  sendPage2HTR = (e) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        this.raw_text_update(lista);
        this.setState({ text_resp_length: lista.length }, () =>
          this.setState({ text_response: lista, loading: false })
        ); //,this.jsPdfGenerator))
      })
      .catch((err) => console.log(err), this.setState({ loading: false }));
  };
  ...
  premilo = () => {
    let lisii = [];
    this.state.bounding_boxes.forEach((box) =>
      lisii.push({ id: box.id, text: "Ferdinando II" })
    );
    this.raw_text_update(lisii);
    this.setState({ text_resp_length: lisii.length }, () =>
      this.setState({ text_response: lisii }, this.jsPdfGenerator)
    );
  };
    ...
    let right_panes = [
      ...
      },
      {
        ...
        pane: {
          ...
          content:
            this.state.pdf !== null ? (
              <PDFObject url={this.state.pdf} style={{ height: "100vh" }} />
            ) : (
              <div>
      ...
      },
      {
        ...
        pane: {
          ...
          content: (
            <TextArea
              onChange={this.occamb}
              placeholder="Raw data output will appear here.."
              style={{ minHeight: "100%", width: "100%", maxHeight: "100%" }}
    ...
    let left_panes = [
      {
        ...
        pane: {
          ...
          content: (
            <LeftCardSinglePage
              file_image_url={this.state.file_image_url}
        ...
        pane: {
          ...
          content: (
            <Tab
              ...
              onTabChange={this.handleTabModelChange}
            ...
            <div className="row h-100 align-items-center">
              <div className=".col-6 .col-md-4">
                ...
                <p className="text-secondary pt-4">
                  Upload a document to visualize its text, starting the
                  recognition using a ML model of your choice
                </p>
              </div>
              <div>
                <button
                  type="button"
                  className="btn btn-outline-dark ml-3 mr-2"
                  onClick={this.SaveImagePDF}
                >
                  Download Results
                </button>
                <button
