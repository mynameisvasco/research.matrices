...
export default class Profile extends React.Component {
  ...
  deleteDataset = (event, index) => {
    ...
    axios.delete(resource, { headers: {} }).then((response) => {
      this.setState({
        list_dataset: update(this.state.list_dataset, {
          $splice: [[index, 1]],
        }),
      });
    });
  ...
  getDatasetFields = (JSON, index) => {
    ...
    for (let i = 0; i < JSON.data.data.length; i++) {
      ...
      for (let u = 0; u < curr_image.annotations.length; u++) {
        let new_box = {
          id: u + 1,
          x: curr_image.annotations[u].x,
          y: curr_image.annotations[u].y,
          width: curr_image.annotations[u].width,
          height: curr_image.annotations[u].height,
        };
    ...
    this.setState({
      list_dataset: update(this.state.list_dataset, {
        [index]: { img: { $set: list_dataset_images } },
      }),
    });
  };
  ...
  render() {
        ...
        your_models.push(
          <Segment key={index} raised style={{ borderRadius: "0px" }}>
            <Modal
              size={"small"}
              key={index}
              trigger={<button className="btn btn-link">{value.name}</button>}
            >
        ...
        your_dataset.push(
          <>
            <Segment key={index} raised style={{ borderRadius: "0px" }}>
              <Modal
                size={"small"}
                onClose={() => this.setState({ is_loading: true })}
                key={index}
                trigger={
                  <button
                    className="btn btn-link"
                    onClick={() => this.getDatasetFields(value.json, index)}
                  >
                    {value.name}
                  </button>
                }
              >
                <Modal.Header style={{ borderRadius: "0px" }}>
                  {value.name}
                </Modal.Header>
                <Modal.Content image>
                  <p>Descrizione</p>
                    ...
                    <>
                      ...
                      <Link
                        to={{
                          ...
                          state: {
                            fromRedirect: this.state.list_dataset[index].img,
                            dataset_id: value.id,
    ...
    const panes = [
        ...
        render: () => (
          <Tab.Pane attached={false}>
            <div className="card-text">
              {your_dataset.length !== 0 ? your_dataset : "NO dataset yet"}
            </div>
          </Tab.Pane>
        ),
      },
      {
        ...
        render: () => (
          <Tab.Pane attached={false}>
            {your_models.length !== 0 ? your_models : "NO Models yet"}
          </Tab.Pane>
        ),
      },
    ];
    return (
      <div>
        <Tab
          menu={{ tabular: false }}
          panes={panes}
          style={{ marginBottom: "10vh", borderRadius: "0px" }}
        />
      </div>
    );
  }
