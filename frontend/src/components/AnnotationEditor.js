...
class AnnotationEditor extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      files: this.props.location.state.fromRedirect.sort(
      ...
      disabled: false,
      dataset_name:
        /* this.props.location.state.datasetInfo.dataset_name ? this.props.location.state.datasetInfo.dataset_name :  */ this
          .props.location.state.dataset_name,
  ...
  handleAddBox = (lista_id, new_boxes, max_id) => {
    ...
    for (let i = 0; i < lista_id.length; i++) {
      let ID = lista_id[i];
      var elem = {};
      var result = this.state.files[
        this.state.index_active_file_in_files
      ].text_response.filter((obj) => {
        return obj.id === ID;
      });
      if (
        result.length > 0 &&
        result[0].text.localeCompare(TO_ANNOTATE) !== 0 &&
        result[0].text.localeCompare("") !== 0
      ) {
    ...
    if (
      JSON.stringify(
        this.state.files[this.state.index_active_file_in_files].boxes
      ) !== JSON.stringify(new_boxes)
    ) {
    ...
    axios
      ...
      .then((response) => {
        ...
        for (
          let i = 0;
          i <
          this.state.files[this.state.index_active_file_in_files]
            .list_active_texts.length;
          i++
        ) {
          ocr_hints.push({
            id: this.state.files[this.state.index_active_file_in_files]
              .list_active_texts[i],
            text: response.data["predictions"][i],
          });
        }
  ...
  updateGT = (new_text_response) => {
    ...
    for (
      let i = 1;
      i <=
      this.state.files[this.state.index_active_file_in_files]["boxes"].length;
      i++
    ) {
  ...
  filter_files = () => {
    ...
    const filter1 = this.state.files.filter(
      (element) => element.text_response.length === 0
    );
    const filter2 = this.state.files.filter(
      (element) =>
        element.is_confirmed === false && element.text_response.length > 0
    );
    const filter3 = this.state.files.filter(
      (element) => element.is_confirmed === true
    );
  ...
  async_set_segmentation_boxes = (index_file, boxes_from_segmentation) => {
    this.setState({
      files: update(this.state.files, {
        [index_file]: { boxes: { $set: boxes_from_segmentation } },
      }),
    });
  };

  deleteTextLine = (id) => {
    let new_boxes = this.state.files[
      this.state.index_active_file_in_files
    ].boxes.filter((obj) => {
      return obj.id !== id;
    });
    this.setState(
      ...
      () => {
        this.setState({
          active_file: this.state.files[this.state.index_active_file_in_files],
        });
      }
    );
  };
  ...
  createDataset = () => {
    ...
    form_data.append(
      "name",
      this.props.location.state.datasetInfo.dataset_name
    );
    ...
    axios
      ...
      .then((response) => {
        ...
        link.setAttribute(
          "download",
          this.props.location.state.datasetInfo.dataset_name + ".zip"
  ...
  sendDatasetToServer = () => {
    ...
    form_data.append(
      "language",
      this.props.location.state.datasetInfo.language
    );
    form_data.append(
      "description",
      this.props.location.state.datasetInfo.description
    );
      ...
      for (const [index, value] of filtered.entries()) {
        items.push(
          ...
          >
            ...
            <Img
              active_id={this.state.index_active_file_in_files}
              personal_index={filtered[index].index}
              src={value.file_url}
              alt=""
            />
            <Icon
          ...
          <div>
            ...
            >
              <div className="container">
                <div>
                  <Button.Group basic widths="5">
                    <Button
                      id="botn"
                      toggle
                      active={this.state.filter === 0}
                      compact
                      onClick={this.togglefilter0}
                    >
                      All
                    </Button>
                    <Button
                    ...
                    <Button
                      id="botn"
                      toggle
                      active={this.state.filter === 2}
                      compact
                      onClick={this.togglefilter2}
                    >
                      <i
                        className="fa fa-check-circle"
                        style={{ color: "#ffae1a" }}
                    ...
                    <Button
                      id="botn"
                      toggle
                      active={this.state.filter === 3}
                      compact
                      onClick={this.togglefilter3}
                    >
                      <i
                        className="fa fa-check-circle"
                        style={{ color: "#7cfc00" }}
                    ...
                    <Button
                      ...
                      onClick={
                        this.state.is_from_creation === true
                          ? this.sendDatasetToServer
                          : this.sendUpdateToServer
                      }
                    >
                      <FaSave style={{ fontSize: "1.5vw" }} />
                    </Button>
                    <button
                ...
                <div className="row">{items.length ? items : null}</div>
              </div>
            </Left>
            <Center
              toggleLeft={this.state.toggleLeft}
            ...
            >
              <div
                className="sticky-button-confirm"
                style={{
                  background: "#f8f9fa",
                  marginBottom: "1vh",
                  marginTop: "1vh",
              ...
              >
                <Button id="confirm" compact basic onClick={this.sendPage2HTR}>
                  OCR Hint
                </Button>
              </div>
              <div
              ...
              {this.state.list_length > 0 ? (
                <TextEditor
                  ...
                  highlight_rect_on_start_text_edit={(id) =>
                    this.setState({ highlighted_box: id })
                  }
                ...
                <div>
                  <div className="bbplaceholder">
                    <img
                      className="img_placeholder_annotation"
                      alt=""
                      src={boundingbox_placeholder}
                    ></img>
                  </div>
