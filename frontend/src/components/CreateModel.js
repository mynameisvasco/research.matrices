...
class CreateModel extends React.Component {
  ...
  handleClick = (e, { id }) => {
    ...
    if (id === "Dataset") {
      this.setState({ open_modal_dataset: true });
    }
    if (id === "Model") {
      this.setState({ open_modal_info: true });
    }
  };
  ...
  handleSubmit = () => {
    ...
    this.setState({ model_fi: model, open_modal_info: false }, () => {
      console.log(this.state.model_fi);
    });
  };
        ...
        your_dataset.push(
          <Segment key={index} raised>
            <button
              ...
              onClick={() =>
                this.setState(
                  { choosed_dataset: { name: value.name, id: value.id } },
                  () => {
                    this.setState({ open_modal_dataset: false });
                  }
                )
              }
            >
      ...
      <div>
            ...
            <div className="row h-100 align-items-center">
              <div className=".col-6 .col-md-4">
                ...
                <p className="text-secondary pt-4">
                  Select a Dataset among the ones in your collection, insert
                  basic ML parameters and start the training
                </p>
              </div>
              <div></div>
            </div>
        ...
        {this.state.is_started ? (
          <div style={{ textAlign: "center", height: "100vh" }}>
            <h1 className="display-4 text-dark mt-5 mb-2">
              Well Done! Your training has started!
            </h1>
            <p className="text-secondary pt-4">
              You will recieve an alert when your model is ready to be used! in
              the meantime enjoy this gif :)
            </p>
          ...
          <Step.Group unstackable ordered widths={3}>
            <Step
              completed={this.state.choosed_dataset !== null}
            ...
            >
              <Step.Content>
                ...
                <Step.Description>
                  Please Choose a Dataset among the ones in your collection
                </Step.Description>
                <br />
                {this.state.choosed_dataset === null ? (
                  <i>No Dataset choosed yet</i>
                ) : (
                  <div>
                    <i>Dataset choosed : </i>{" "}
                    <b>{this.state.choosed_dataset.name}</b>
                  </div>
                )}
                <br />
                <br />
                <Modal
                  open={this.state.open_modal_dataset === true}
                  onClose={() => this.setState({ open_modal_dataset: false })}
                  size={"small"}
                >
                  <Modal.Header>Your Dataset List</Modal.Header>
                  <Modal.Content>{your_dataset}</Modal.Content>
                </Modal>
              </Step.Content>
            </Step>
            <Step
              completed={this.state.model_fi !== null}
              active={active === "Model"}
              link
              onClick={this.handleClick}
              id="Model"
            >
              <Step.Content>
                ...
                <Step.Description>
                  Please insert ML parameters and the model basic informations
                </Step.Description>
                {this.state.model_fi !== null ? model_info : null}
                <Modal
                  open={this.state.open_modal_info}
                  onClose={() => this.setState({ open_modal_info: false })}
                  size={"small"}
                >
            ...
            >
              <Step.Content>
                ...
                <Button
                  disabled={
                    this.state.choosed_dataset === null ||
                    this.state.model_fi === null
                  }
                  onClick={this.sendTrainingInfo}
                >
                  {" "}
                  Press to send
                </Button>
